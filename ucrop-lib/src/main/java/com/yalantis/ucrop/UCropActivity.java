...
public class UCropActivity extends AppCompatActivity{
    ...
    private void initView(Intent intent) {
        String toolbarTitleName = intent.getStringExtra(UCrop.TOOLBAR_TITLE_NAME);
        String rightButtonName = intent.getStringExtra(UCrop.TOOLBAR_RIGHT_NAME);
        mTitleBar = (Toolbar) findViewById(R.id.toolbar);

        ivBack = (ImageView) findViewById(R.id.iv_back);
        ivNext = (TextView) findViewById(R.id.iv_next);
        tvTitleName = (TextView) findViewById(R.id.toolbar_title);
        ...
        if(!TextUtils.isEmpty(toolbarTitleName)) {
            tvTitleName.setText(toolbarTitleName);
        }
        if(!TextUtils.isEmpty(rightButtonName)) {
            ivNext.setText(rightButtonName);
        }
        setCustomStyle();
    }
    ...
    private void setCustomStyle() {
        ...
        if (config.getTitleBarBgColor() != 0){
            mTitleBar.setBackgroundColor(ContextCompat.getColor(this,config.getTitleBarBgColor()));
        }

        if(config.getBarTitle() != null)  {
            tvTitleName.setText(config.getBarTitle());
        }

        if (config.getTitleTextColor() != 0){
            tvTitleName.setTextColor(ContextCompat.getColor(this,config.getTitleTextColor()));
        }

        if (config.getTitleTextSize() != 0){
            tvTitleName.setTextSize(config.getTitleTextSize());
        }


        if (config.getTitleBarRightBtnColor() != 0){
            ivNext.setBackgroundResource(config.getTitleBarRightBtnColor());
        }


        if (config.getTitleBarBackBtnImage() != 0){
            ivBack.setImageResource(config.getTitleBarBackBtnImage());
        }

        if(config.getTitleBarRightBtnTextColor() != 0){
            ivNext.setTextColor(ContextCompat.getColor(this,config.getTitleBarRightBtnTextColor()));
        }

        if(config.getRightBtnTextSize() != 0){
            ivNext.setTextSize(config.getRightBtnTextSize());
        }

        if(config.getBarRightText() != null) {
            ivNext.setText(config.getBarRightText());
        }
...
//    }
    ...
    private void setupViews(@NonNull Intent intent) {
      ...
      //  mActiveWidgetColor = intent.getIntExtra(UCrop.Options.EXTRA_UCROP_COLOR_WIDGET_ACTIVE, ContextCompat.getColor(this, R.color.ucrop_color_statusbar));
        mToolbarWidgetColor = intent.getIntExtra(UCrop.Options.EXTRA_UCROP_WIDGET_COLOR_TOOLBAR, ContextCompat.getColor(this, R.color.ucrop_color_statusbar));

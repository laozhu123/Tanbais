...
public class GestureCropImageView extends CropImageView {
    ...
    private int mDoubleTapScaleSteps = 1;
    ...
    private int mDoubleTapScaleCurentSteps = 0;
    private float mLastScale = getCurrentScale();
    ...
    private class GestureListener extends GestureDetector.SimpleOnGestureListener {
        ...
        public boolean onDoubleTap(MotionEvent e) {
            if (mDoubleTapScaleCurentSteps == 0){
                mLastScale = getCurrentScale();
            }
            if (mDoubleTapScaleCurentSteps < mDoubleTapScaleSteps){
                zoomImageToPosition(getDoubleTapTargetScale(), e.getX(), e.getY(), DOUBLE_TAP_ZOOM_DURATION);
                mLastX = e.getX();
                mLastY = e.getY();
                mDoubleTapScaleCurentSteps ++;
            }else {
                zoomImageToPosition(getCurrentScale()*2/3f, mLastX, mLastY, DOUBLE_TAP_ZOOM_DURATION);
                mDoubleTapScaleCurentSteps = 0;
            }
